"""
Windowed Mamba Training — Full 62-Channel SEED-IV.

With 62 channels (vs 4), the model has full spatial brain topology,
enabling it to learn frontal asymmetry, occipital-temporal connectivity,
and all other spatial emotion markers documented in literature.

Expected accuracy: 65-80% (subject-dependent), vs 41-45% with 4 channels.

Usage:
    python train_mamba_62ch.py --dataset_path /path/to/seed_iv --epochs 100

    Kaggle:
    python train_mamba_62ch.py \
        --dataset_path /kaggle/input/datasets/phhasian0710/seed-iv/seed_iv \
        --sessions 1 2 3 \
        --epochs 100

    Subject-independent:
    python train_mamba_62ch.py --dataset_path /path/to/seed_iv \
        --mode sub_indep --n_test_subj 3 --n_val_subj 2 --epochs 100
"""
...
from seed62_loader import load_seediv_62ch, NUM_CHANNELS

# Reuse mamba model and windowed dataset from mamba/
...
def main():
    ...
    # Model — note in_channels=62
    parser.add_argument('--d_model',  type=int,   default=128,
                        help='Hidden dim (default: 128 — more channels = more capacity needed)')
    parser.add_argument('--n_layers', type=int,   default=4,
                        help='Mamba blocks (default: 4)')
    ...
    parser.add_argument('--lr',           type=float, default=5e-5,
                        help='Lower LR for 62ch (more params, needs stable gradient)')
    ...
    print(f"\n{'='*60}")
    print(f"62-Channel Mamba — SEED-IV  (targeting 70%+)")
    print(f"  Channels   : {NUM_CHANNELS}")
    print(f"  Window     : {args.window_sec}s ({window_size} samples → ~{window_size//16} Mamba steps)")
    print(f"  Mode       : {args.mode}")
    print(f"  d_model={args.d_model}, layers={args.n_layers}, dropout={args.dropout}")
    print(f"  lr={args.lr}, wd={args.weight_decay}, smooth={args.label_smooth}")
    print(f"  Device     : {device}")
    print(f"{'='*60}")

    # Load data
    print(f"\nLoading SEED-IV (62 channels)...")
    t0 = time.time()
    trials, labels, subject_ids, session_ids = load_seediv_62ch(
    ...
    if args.mode == 'sub_dep':
        train_ds, val_ds, test_ds, _ = create_windowed_splits(
        ...
        print(f"\n  Subject split:")
        print(f"    Train ({len(train_s)}): {sorted(train_s)}")
        print(f"    Val   ({len(val_s)}):   {sorted(val_s)}")
        print(f"    Test  ({len(test_s)}):  {sorted(test_s)}")
    ...
    # Model — same MambaEEGClassifier, just in_channels=62
    model = MambaEEGClassifier(
        in_channels=NUM_CHANNELS, num_classes=4,
    ...
    print(f"\n{'='*60}")
    print(f"RESULTS — 62ch Mamba on SEED-IV ({args.mode})")
    print(f"{'='*60}")
    ...
    print_report(te_labels, te_preds, title="SEED-IV 62ch Test")

    ckpt_dir  = os.path.join(os.path.dirname(__file__), 'checkpoints')
    os.makedirs(ckpt_dir, exist_ok=True)
    ckpt_path = os.path.join(ckpt_dir, 'best_model_62ch.pt')
    torch.save({
